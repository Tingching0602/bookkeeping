<!--
建立一個 Vue3 + TypeScript 頁面：
1. 包含輸入每日花費金額、選擇分類、與新增按鈕
2. 將資料以列表顯示在下方
3. 使用 Pinia 或 Composition API 管理狀態
-->
<template>
  <div class="expenses">
    <h1>每日花費記錄</h1>
    <div class="input-section">
      <div class="input-group">
        <input v-model.number="amount" type="number" min="0" step="any" placeholder="輸入花費金額" class="amount-input" />
  ...
  <button @click="addExpense" class="btn btn-add">新增</button>
      </div>
    </div>
    <div class="day-group" v-for="group in groupedExpenses" :key="group.key">
      <div class="day-header">
        <span class="day-label">{{ group.label }}</span>
        <span class="day-total">合計 ${{ group.total }}</span>
      </div>
      <ul class="expense-list">
        <li v-for="expense in group.items" :key="expense.id">
              ...
              <div class="amount-area">
                <template v-if="editingId === expense.id">
                  <input
                    ...
                    class="edit-input"
                    placeholder="輸入金額"
                    @keyup.enter="updateAmount(expense.id)"
                  />
                  <div class="edit-buttons">
                    <button @click="updateAmount(expense.id)" class="btn btn-save">更新</button>
                ...
                <template v-else>
                  <div class="amount-display">
                    ...
                    <div class="action-buttons">
                      <button @click="startEdit(expense)" class="btn btn-edit">編輯</button>
                      <button @click="handleDelete(expense.id)" class="btn btn-delete">刪除</button>
                    </div>
                  </div>
                </template>
              </div>
...
function startEdit(expense: { id: number, amount: number }) {
...
function updateAmount(id: number) {
  if (editAmount.value !== null && editAmount.value > 0) {
    store.updateExpense(id, editAmount.value)
    cancelEdit()
  }
}
...
function addNewCategory() {
  ...
  category.value = trimmedCategory // 使用修剪過的值
  newCategory.value = ''
}
