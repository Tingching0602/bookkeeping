...
function buildChart() {
  ...
  const config: any = {
    ...
    options: {
      ...
      // add extra layout padding for pie so slices/datalabels are not clipped
      layout: {
        padding: {
          top: 0,
          right: 0,
...
watch([() => store.expenses.length, chartType, startDate, endDate], () => {
  buildChart()
})
</script>
...
.chart-card canvas {
  flex: 1;
  max-height: calc(100% - 50px); /* 扣除控制項高度和間距 */
  width: 100%;
  display: block;
}
